  ...
  db.query(
    ...
    function (err, results) {
      if (results.length > 0) {
        ...
        db.query(
          `INSERT INTO ${dbTable} (staffId,eventName,eventDescription,eventLocation,eventStartDate,eventStartTime,eventEndDate,eventEndTime,isAllDay) VALUES(?,?,?,?,?,?,?,?,?)`,
          [
            req.body.staffId,
            req.body.eventName,
...
function validateEvents(event) {
  const schema = {
    staffId: Joi.number().min(0).max(250).required(),
