...
router.get("/:conversation_id", [auth], async (req, res) => {
  db.query(
    `SELECT U.id,R.id,R.created,R.message,U.firstName,U.lastName,U.email,U.avatarUrl FROM staff U,conversation_reply R WHERE R.conversationId = ? AND R.staffId = U.id ORDER BY R.id ASC`,
...
router.post("/", [auth], async (req, res) => {
  ...
  db.query(
    `INSERT INTO ${dbTable} (message,staffId,conversationId) VALUES(?,?,?)`,
